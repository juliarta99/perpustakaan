...
<div class="w-full flex justify-end">
      <div class="pt-24 px-5 w-full lg:w-3/4 xl:w-4/5 pb-10">
            <form action="/dashboard/user" class="flex flex-col" method="post">
                  ...
                  <input value="{{ old('is_petugas', '1') }}" class="w-full py-2 px-4 rounded-md bg-gray-100 @error('is_petugas') border-2 border-red-500 solid @enderror" type="checkbox" name="is_petugas" id="is_petugas">
                  @error('is_petugas')
                      <div class="mb-3 text-sm text-red-500 lg:text-md">{{ $message }}</div>
                  @enderror
