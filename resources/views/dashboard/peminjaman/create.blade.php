...
<div class="w-full flex justify-end">
    <div class="pt-24 px-5 w-full lg:w-3/4 xl:w-4/5 pb-10">
        <div class="w-full">
            <form action="/dashboard/peminjaman" autocomplete="off" method="post" class="flex flex-col">
                @csrf

                <label for="id_buku" class="text-sm mt-2 md:text-md">Buku</label>
                <input type="text" list="listBuku" name="id_buku" id="id_buku" value="{{ old('id_buku') }}" class="w-full py-2 px-4 rounded-md bg-gray-100 @error('id_buku') border-2 border-red-500 solid @enderror">
